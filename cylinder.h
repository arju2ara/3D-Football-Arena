...
private:
    void buildCoordinatesAndIndices()
    {
        ...
        // avoid clashing with any PI macro; use constexpr name and float-only math
        constexpr float kPi = 3.14159265358979323846f;
        float sectorStep = 2.0f * kPi / static_cast<float>(sides);
        ...
        for (int i = 0; i <= sides; ++i)
        {
            ...
            x = radius * std::cosf(angle);
            z = radius * std::sinf(angle);
